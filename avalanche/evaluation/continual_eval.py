...
from avalanche.evaluation.continual_eval_metrics import TrackerPluginMetric, TrackingCollector
...
class ContinualEvaluationPhasePlugin(BaseSGDPlugin):
    ...
    def set_subset_idxs(self):
        ...
        if self.max_task_subset_size is not None:
            ...
            for exp, task_label in zip(self.eval_stream, self.eval_stream_task_labels):
                ...
                print("\nContinual eval subset (first 100): exp:", task_label, task_subset_idxs[:100])
    ...
    def before_training_iteration(self, strategy: 'BaseStrategy') -> 'MetricResult':
        ...
        #self.set_subset_idxs()  # NOTE: This is not called here, but in the constructor.
        ...
        # Turn off tracking if max iterations reached
        strategy.tracking_collector.is_tracking_iteration = (
            (strategy.clock.train_epoch_iterations % self.mb_update_freq == 0 
             and (strategy.clock.train_exp_iterations < self.eval_max_iterations 
                  or self.eval_max_iterations == -1
                 )
            )
    ...
    def forward_data(self, dataset, strategy, tracking_task, collect_feats=False, subset_idxs: list = None):
        ...
        if subset_idxs is not None:
            dataset = AvalancheDataset(dataset, indices=subset_idxs)
        ...
        for it, mbatch in enumerate(dataloader):
            ...
            feats_batch = strategy.model.get_features(col.x, col.task_id) 
            #col.preds_batch = strategy.model.forward_classifier(feats_batch, task_labels=col.task_id)
            col.preds_batch = avalanche_forward(strategy.model.train_classifier, feats_batch, col.task_id)

            #col.preds_batch = strategy.model(col.x, col.task_id)
            #col.preds_batch = avalanche_forward(strategy.model, col.x, col.task_id) # automatically handles dynamic modules
            #feats_batch = strategy.model.features
            loss_batch += criterion(col.preds_batch, col.y)  # Criterion avgs over batch dim

            # Collect features
            if collect_feats:
                raise NotImplementedError("Removed for debugging purpose")
