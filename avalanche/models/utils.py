...
class FeatureExtractorModel(BaseModel, MultiTaskModule):
    """
    Feature extractor that additionnaly stores the features,
    and allows to get only the features from the model.
    """
    ...
    def forward(self, x, task_labels=None):
        ...
        #self.features = x.clone().detach()
        #x = self.train_classifier(self.features)
        x = avalanche_forward(self.train_classifier, x, task_labels)
        return x
    
    def get_features(self, x, task_labels=None):
        #self.features = self.feature_extractor(x) # NOTE: could also not store the features...
        x = avalanche_forward(self.feature_extractor, x, task_labels)
        #self.features = x.clone().detach()
        return x
