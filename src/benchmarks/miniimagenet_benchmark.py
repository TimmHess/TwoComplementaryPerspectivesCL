...
def get_miniimgnet_dataset(rootpath, transform):
    """
    ...
    with open(f"{rootpath}/miniImageNet.pkl", "rb") as f:
    ...
    return XYDataset(train_x, train_y, transform=transform), XYDataset(test_x, test_y, transform=transform)


def SplitMiniImageNet(rootpath, 
                      n_experiences=20, 
    ...
    :param rootpath: Root path of the downloaded dataset.
    ...
    if preprocessed:
        train_set, test_set = _get_preprocessed_split_mini_imagenet(rootpath)
    else:
        train_set, test_set = _get_mini_imagenet_dataset(rootpath)
...
def _get_preprocessed_split_mini_imagenet(rootpath):
    """
    Use preprocessed train (500 imgs) /test (100 imgs) split in Numpy Array. For 100 classes.
    Download: https://github.com/yaoyao-liu/mini-imagenet-tools
    """
