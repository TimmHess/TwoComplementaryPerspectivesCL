...
class LwFStandardPlugin(StrategyPlugin):
    ...
    def penalty(self, out, x, alpha, curr_model, t=None):
        """
        ...
        if self.prev_model is None:
            ...
            if isinstance(self.prev_model.classifier, MultiTaskModule):
                with torch.no_grad():
                ...
                # Updated version that only re-runs the head not the entire model
                #y_curr = avalanche_forward(curr_model.classifier, curr_model.last_features, None) #NOTE: this should only forward the heads
                # Even further updated version that only re-runs heads that are missing not all heads
                prev_heads_idxs = list(y_prev.keys())
                y_curr = {}
                for head_idx in prev_heads_idxs:
                    #print("head_idx", head_idx, type(head_idx))
                    y_curr[head_idx] = avalanche_forward(curr_model.classifier, curr_model.last_features, task_labels=t.fill_(head_idx))
                # TODO: Even better would be to only forward the heads that are missing (currently its all)
                # Instead of None we give a "tensor of lables", which we can assume to be all labes from 0 to the current task-1
            ...
            for task_id in y_prev.keys():
                ...
                #print("prev_classes", self.prev_classes)
